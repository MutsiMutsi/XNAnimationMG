...
namespace Microsoft.Xna.Framework.Content.Pipeline
{
	...
	[ContentImporter(
		...
		".ndo", // Izware Nendo
		DisplayName = "ArcturusModelImporter", DefaultProcessor = "ModelProcessor")]
	public class ArcturusModelImporter : ContentImporter<NodeContent>
	{
		...
		/// Default constructor.
		/// </summary>
		public ArcturusModelImporter()
			: this("ArcturusModelImporter", false)
		{
		}

		internal ArcturusModelImporter(string importerName, bool xnaCompatible)
		{
		...
		private GeometryContent CreateGeometry(MeshContent mesh, Mesh aiMesh)
		{
			...
			if (aiMesh.HasBones)
			{
				...
				BoneWeightCollection[] xnaWeights = new BoneWeightCollection[vertexCount];
				for (int i = 0; i < xnaWeights.Length; i++)
					xnaWeights[i] = new BoneWeightCollection();

				bool missingBoneWeights = false;
				for (int boneIndex = 0; boneIndex < aiMesh.BoneCount; boneIndex++)
				{
					Bone bone = aiMesh.Bones[boneIndex];
					for (int i = 0; i < bone.VertexWeights.Count; i++)
						xnaWeights[bone.VertexWeights[i].VertexID].Add(new BoneWeight(bone.Name, bone.VertexWeights[i].Weight));
				}

				for (int i = 0; i < vertexCount; i++)
				{
					if (xnaWeights[i].Count == 0)
					{
						// No bone weights found for vertex. Use bone 0 as fallback.
						missingBoneWeights = true;
						xnaWeights[i].Add(new BoneWeight(aiMesh.Bones[0].Name, 1));
					}
				}
