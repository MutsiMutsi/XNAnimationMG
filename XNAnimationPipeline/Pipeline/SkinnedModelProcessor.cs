...
namespace XNAnimationPipeline.Pipeline
{
	...
	public class SkinnedModelProcessor : ContentProcessor<NodeContent, SkinnedModelContent>
	{
		...
		[DisplayName("Skin Animations"),
		Description("If true will no skip trying to process animations, useful if animations are separate files."),
		DefaultValue(typeof(bool), "false")]
		public virtual bool SkipAnimations
		{
		...
		public override SkinnedModelContent Process(NodeContent input, ContentProcessorContext context)
		{
			...
			AnimationClipContentDictionary animationClipDictionary = new AnimationClipContentDictionary(new Dictionary<string, AnimationClipContent>());
			if (!skipAnimations)
			{
				animationClipDictionary = ProcessAnimations(input, rootBone.Animations, skinnedModelBoneCollection, context);
			}
