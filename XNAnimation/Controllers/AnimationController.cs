...
namespace XNAnimation.Controllers
{
	...
	public class AnimationController : IAnimationController, IBlendable
	{
		...
		private int currentChannelKeyFrame;

		//Additive Animation;
		private AdditiveAnimationBlendMode additiveBlendMode;
		private AnimationClip additiveClip;
		private TimeSpan additiveTime;
		private float additiveWeight;
		private HashSet<int> additiveBoneIndices = null;
		...
		{
			get { return additiveWeight; }
			set { additiveWeight = MathHelper.Clamp(value, 0f, 1f); }
		}
		...
		private void UpdateChannelPoses()
		{
			...
			TimeSpan longestChannel = new TimeSpan();
			...
			for (int i = 0; i < localBonePoses.Length; i++)
			{
				...
				if (animationClip.Channels.TryGetValue(animationChannelName, out animationChannel))
				{
					...
					if (animationChannel[animationChannel.Count - 1].Time > longestChannel)
					{
						currentKeyframe = currentChannelKeyFrame;
						longestChannel = animationChannel[animationChannel.Count - 1].Time;
					}
				}
				...
				string boneName = skeleton[i].Name;

				// --- Pose Overwrite blending ---
				if (additiveBlendMode == AdditiveAnimationBlendMode.PoseOverwrite)
				{
					if (additiveClip != null && additiveWeight > 0f &&
						additiveBoneIndices.Contains(i) &&
						additiveClip.Channels.TryGetValue(boneName, out var overrideChannel))
					{
						InterpolateChannelPose(overrideChannel, additiveTime, out var overridePose);
						// Override blend: use overridePose directly or blend it in by weight
						localBonePoses[i] = Pose.Interpolate(localBonePoses[i], overridePose, additiveWeight, InterpolationMode.Linear, InterpolationMode.Linear, InterpolationMode.Linear);
					}
				}
				else if (additiveBlendMode == AdditiveAnimationBlendMode.PoseAdd)
				{
					// --- Additive blending ---
					if (additiveClip != null && additiveWeight > 0f &&
						additiveBoneIndices.Contains(i) &&
						additiveClip.Channels.TryGetValue(boneName, out var additiveChannel))
					{
						// Sample additive pose and bind pose
						InterpolateChannelPose(additiveChannel, additiveTime, out var additivePose);
						Pose bindPose = skeleton[i].BindPose;

						// Compute delta from bind pose
						Vector3 deltaTranslation = additivePose.Translation - bindPose.Translation;
						Quaternion deltaRotation = Quaternion.Concatenate(additivePose.Orientation, Quaternion.Inverse(bindPose.Orientation));
						Vector3 deltaScale = additivePose.Scale - bindPose.Scale;

						// Apply deltas additively
						localBonePoses[i].Translation += deltaTranslation * additiveWeight;
						localBonePoses[i].Orientation = Quaternion.Slerp(localBonePoses[i].Orientation,
							Quaternion.Concatenate(deltaRotation, localBonePoses[i].Orientation),
							additiveWeight);
						localBonePoses[i].Scale += deltaScale * additiveWeight;
					}
				}
			}
		}
			...
			out Pose outPose)
		{
			if (translationInterpolation == InterpolationMode.None &&
				orientationInterpolation == InterpolationMode.None &&
					scaleInterpolation == InterpolationMode.None)
			{
				currentChannelKeyFrame = animationChannel.GetKeyframeIndexByTime(animationTime);
				outPose = animationChannel[currentChannelKeyFrame].Pose;
			}
			else
			{
				currentChannelKeyFrame = animationChannel.GetKeyframeIndexByTime(animationTime);
				...
				if (loopEnabled)
				{
					nextKeyframeIndex = (currentChannelKeyFrame + 1) % animationChannel.Count;
				}
				else
				{
					nextKeyframeIndex = Math.Min(currentChannelKeyFrame + 1, animationChannel.Count - 1);
				}

				AnimationChannelKeyframe keyframe1 = animationChannel[currentChannelKeyFrame];
				...
				if (currentChannelKeyFrame == (animationChannel.Count - 1))
		...
		private void AddBoneAndDescendants(int boneIndex, HashSet<int> outputSet)
		{
			outputSet.Add(boneIndex);
			for (int i = 0; i < skeleton.Count; i++)
			{
				if (skeleton[i].Parent?.Index == boneIndex)
				{
					AddBoneAndDescendants(i, outputSet);
				}
			}
		}

		/// <summary>
		/// Allows you to selectively blend in animations by bone from another animation
		/// </summary>
		/// <param name="clip">Which animation to blend in</param>
		/// <param name="time">The animation time of the blend in animation</param>
		/// <param name="weight">How much do you want to blend in</param>
		/// <param name="boneNames">Which bones do you want to blend in</param>
		/// <param name="addDescendants">If you set this to true, you only have to provide the names of parent bones, for example if you provide knee, it will automatically also include all children; feet, toes etc.</param>
		/// <param name="blendMode">PoseOverwite; for big motions such as holding a weapon while being able to blend with idle, walk, run etc.
		/// PoseAdd; for small independent motions such as adding eyes blinking, mouth opening etc that can be added on top of existing transforms.</param>
		public void SetAdditiveAnimation(AnimationClip clip, TimeSpan time, float weight, string[] boneNames, bool addDescendants, AdditiveAnimationBlendMode blendMode)
		{
			additiveBlendMode = AdditiveAnimationBlendMode.PoseOverwrite;

			additiveClip = clip;
			additiveTime = time;
			additiveWeight = MathHelper.Clamp(weight, 0f, 1f);

			// Precompute bone indices
			additiveBoneIndices = new HashSet<int>();
				...
				foreach (string name in boneNames)
				{
					for (int i = 0; i < skeleton.Count; i++)
					{
						if (skeleton[i].Name == name)
						{
							...
							AddBoneAndDescendants(i, additiveBoneIndices);

							additiveBoneIndices.Add(i);
							break;
						}
					}
				}
